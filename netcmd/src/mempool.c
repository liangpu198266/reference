...
static memNode_t *mpNewMemNode(uint32 type)
{
  ...
  if (node == NULL)
  {
    NETCMD_ERROR("mempool malloc mem node malloc fail");
    return NULL;
  }
  ...
  switch (type)
  {
    case MEMPOOL_TYPE_SMALL:
      ...
      {
        NETCMD_ERROR("mempool malloc mem small size malloc fail");
        return NULL;
      }
      break;
    ...
    case MEMPOOL_TYPE_MIDDLE:
      ...
      if (node->memAddr == NULL)
      {
        NETCMD_ERROR("mempool malloc mem middle size malloc fail");
        return NULL;
      }      
      break;
    ...
    case MEMPOOL_TYPE_BIG:
      ...
      if (node->memAddr == NULL)
      {
        NETCMD_ERROR("mempool malloc mem big size malloc fail");
        return NULL;
      } 
      break;

    default:
      NETCMD_ERROR("mempool memaddr no malloc");
      return NULL;
...
static memNode_t *mpNewLargeMemNode(uint32 length)
{
  ...
  if (node->memAddr == NULL)
  {
    NETCMD_ERROR("mempool malloc mem large size malloc fail");
    return NULL;
  }
...
static memNode_t *mpCheckNodeByType(uint32 type)
{
  switch (type)
  {
    ...
    default:
      NETCMD_ERROR("mp Check Node fail");
      return NULL;  
  }
...
static memNode_t *mpNewOneNodeByType(uint32 type, uint32 length, mempoolNode_t *poolList)
{
  ...
  if (type == MEMPOOL_TYPE_LARGE)
  {
    ...
    if (node == NULL)
    {
      NETCMD_ERROR("mempool malloc mem node malloc fail at large");
      return NULL;
    }
  ...
  else
  {
    ...
    if (node == NULL)
    {
      NETCMD_ERROR("new malloc Node fail");
    }
  }
  ...
  list_for_each_entry_safe(node, tmp, &poolList->poolhead, memNode) 
  {
    if (freeNode == node)
    {
      if (poolList->status == POOL_OVER)
      {
        ...
        NETCMD_DBG("mp free Node over type is %d, length is %d", node->type, node->realLength);
        ...
        {
          NETCMD_ERROR("node->memAddr is empty");
        } 
      }
      else
      {
        NETCMD_DBG("mp free normal Node type is %d, length is %d", node->type, node->realLength);
        pthread_mutex_lock(&gsMemPoolMutex);
        node->realLength = 0;
        node->mark = MEM_NO_USE;
        memset(node->memAddr, 0x0, poolTypeLists[node->type].size);
...
memNode_t *mpMalloc(uint32 length)
{
  ...
  if ((node = mpCheckNodeByType(type)) == NULL)
  {
    ...
    if (node == NULL)
    {
      NETCMD_ERROR("mp mpNewNodeByType fail");
      return NULL;  
    }

    NETCMD_DBG("mp new Node type is %d, length is %d", node->type, node->realLength);
  }
  else
  {
    node->realLength = length;
    node->mark = MEM_USED;
    NETCMD_DBG("mp old Node type is %d, length is %d", node->type, node->realLength);
  }

  ret = mpSetPoolStatus(type);
  if (ret == TASK_ERROR)
  {
    NETCMD_ERROR("mp mpSetPoolStatus fail");
    return NULL;
  }
...
{
  NETCMD_DBG("netcmd mem pool init\n");
