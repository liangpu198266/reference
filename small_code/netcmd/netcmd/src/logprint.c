...
{
  openlog("netcmd", LOG_CONS | LOG_PID, 0);
...
int8 initFileLogSystem(void)
{
  ...
  if(!log_fp) 
  {
    fprintf(stderr,"failed open log error id %d, content: %s \n",errno, strerror(errno));
    return TASK_ERROR; 
  }
...
int32 netcmdPrintf(int prio, const char *func, const char *fmt, ...)
{  
  va_list ap;  
  ...
  if(log_fp)
  {
    ...
    if(flock(fd,LOCK_EX) == 0)
    {
      ...
      {
        fprintf(stderr,"unlock failed id %d : %s\n",errno, strerror(errno));
      }
    }
    else
    {
      fprintf(stderr,"lock failed id %d : %s \n",errno, strerror(errno));
    }
